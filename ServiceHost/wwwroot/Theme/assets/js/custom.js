...
function retQorbMoasList(id) {

    var getid = id;
    const settings = {
        "url": "/api/qorbmoas/" + getid,
        "method": "Post",
        "timeout": 0,
        "headers": {
        "Content-Type": "application/json"
...
function retQorbMoasPorList(id) {

    var getid = id;
    const settings = {
        "url": "/api/qorbmoaspor/" + getid,
        "method": "Post",
        "timeout": 0,
        "headers": {
            "Content-Type": "application/json"
    ...
    $.ajax(settings).done(function (data) {
        if (data.status == "success") {
            $('#QorbMoasPor').empty();
            var s = '<option value="-1">لطفا پروژه را انتخاب نمایید</option>';
            $("#QorbMoasPor").append(s);
            $.each(data.qorbmoaspors,
                function (i, item) {
                    $('#QorbMoasPor').append($("<option></option>")
                        .val(item.value).html(item.text));
                });
        }
    });
...
function retQorbMoasPorKarList(id) {

    var getid = id;
    const settings = {
        "url": "/api/qorbmoasporkar/" + getid,
        "method": "Post",
        "timeout": 0,
        "headers": {
            "Content-Type": "application/json"
    ...
    $.ajax(settings).done(function (data) {
        if (data.status == "success") {
            $('#QorbMoasPorKar').empty();
            var s = '<option value="-1">لطفا کارگاه را انتخاب نمایید</option>';
            $("#QorbMoasPorKar").append(s);
            $.each(data.qorbmoasporkars,
                function (i, item) {
                    $('#QorbMoasPorKar').append($("<option></option>")
                        .val(item.value).html(item.text));
                });
        }
    });
